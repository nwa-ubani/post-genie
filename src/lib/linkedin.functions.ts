...
export const getLinkedInAuthUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!clientId) throw new Error("LinkedIn is not configured yet. Add LINKEDIN_CLIENT_ID and LINKEDIN_CLIENT_SECRET in Settings → Secrets.");
    const origin = originFromEnv();
    if (!origin) throw new Error("Set PUBLIC_APP_URL to your app's public URL so LinkedIn can redirect back.");
...
export async function exchangeLinkedInCode(code: string, redirectUri: string) {
  ...
  return r.json() as Promise<{ access_token: string; expires_in: number; refresh_token?: string; id_token?: string }>;
}
...
export async function publishToLinkedIn(accessToken: string, memberUrn: string, text: string) {
